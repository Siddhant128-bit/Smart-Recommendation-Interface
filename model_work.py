...
def model_inference(movie_name,example_date,dataframe_ts,parent_directory,user_name):
    ...
    try:
        ...
        if synopsis_flag:
            embedding=gms.get_movie_synopsis_embedding(movie_name,embedder)
        else:
            embedding=gms.get_movie_summary_embedding(movie_name,embedder)
        ...
        try:
            ...
            return {'title':movie_name,'release date': example_date,'hype score': f'In between {trend_scores}','minimum_view': f'{results_list[0]*pred_factor:.2f}k','avg_view':f'{results_list[1]*pred_factor:.2f}k ','max':f'{results_list[2]*pred_factor:.2f}k'}
